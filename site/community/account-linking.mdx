...
description: "Bind a Discord user to a Minecraft account — shared with TriBridge for guild-chat attribution."
---

Binds a Discord user to a Minecraft account. The link is what lets the community bot and
[TriBridge](/tribridge) show the same person on both sides of the guild-chat bridge.
...
`MinecraftLink`, one row per member, related to `User.id` rather than to the Discord id. It holds the
canonical name Mojang returned and the account's UUID.
...
membership — that check lives on TriBridge's `/link`, which needs a connected Minecraft bot. Anyone can
therefore link any real Minecraft name here; an admin undoes a wrong one with `/unlink user:@them`.
...
Members are fetched **one id at a time**, not with a bulk roster fetch.

The bot needs **Manage Roles**, and its own highest role must rank above the link role. `/linkrole set`
refuses up front when either is untrue.

## What TriBridge does with a link
...
attributed to their Minecraft name.
...
See [Shared database](/community/shared-database) for the full contract.
