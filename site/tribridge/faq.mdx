...
You have not [linked your account](/tribridge/account-linking) yet. Run `/link <your Minecraft username>`.
...
❓ — you used a [guild tag](/tribridge/guild-tags) nobody recognises. Your message went to every guild anyway, `!nope`
...
player. The bot says which. See [Networth](/tribridge/networth).
...
[global profile change](/tribridge/global-profile-change) works on — but it does not relay them anywhere, and it does not
store them.
...
each other; see [Guild-to-guild bridging](/tribridge/guild-to-guild-bridging).
...
[Admin roles](/tribridge/admin-roles) for why that is a security property rather than a limitation.
...
Relayed messages are not stored. Disguised messages **are** recorded in the [audit channel](/tribridge/auditing), because
...
`/request` opens a form — see [Feature requests](/tribridge/feature-requests). Bugs in the bot itself go to
[Issues](https://github.com/TheHypixelGuardians/TriBridge/issues).
