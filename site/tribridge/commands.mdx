...
Double the `!` to send one literally: `!!nw x` arrives as the text `!nw x`. See [Networth](/tribridge/networth).

Also typed rather than slashed: **[guild tags](/tribridge/guild-tags)**. `!sb hey` sends to one guild only.
...
| `/networth [username]` | A player's SkyBlock networth. No username uses your [link](/tribridge/account-linking) |
...
See [Account linking](/tribridge/account-linking) and [Link role](/tribridge/link-role).

## Management

Every command in this section is 🔒 bot-admin only. The six that act on a Hypixel guild take an optional
`guild` with autocomplete; leave it out and the [default guild](/tribridge/hypixel-guilds) is used.
...
| `/adminpanel`                         | Open the [admin panel](/tribridge/admin-panel)                                    |
| `/adminrole add\|remove <role>`       | ⛔ Configure the bot-admin roles                                        |
| `/auditchannel set <channel> [guild]` | Set where [audit](/tribridge/auditing) entries go                                 |
...
🔒 The whole [Hypixel guild registry](/tribridge/hypixel-guilds), in one command.
...
`officerchannel` opens a **two-way** [officer chat](/tribridge/officer-chat) channel: everyone who can post in it is
...
See [Feature requests](/tribridge/feature-requests).
...
- **Commands from any other Discord server are refused**, even from someone who is an administrator there.
  See [Admin roles](/tribridge/admin-roles).

## Next

- [Using the bridge](/tribridge/using-the-bridge)
- [Admin roles](/tribridge/admin-roles)
- [Adding a command](/tribridge/adding-a-command)
