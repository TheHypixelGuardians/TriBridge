---
title: "Global Profile Change"
description: "Temporarily disguise everyone's messages as one member."
---
...
Read [Auditing](/tribridge/auditing) before running one live.

Set it up from [`/adminpanel`](/tribridge/admin-panel) → **Global Profile**.

## Setting one up

1. **Pick the member** everybody will appear as. If they have a [linked account](/tribridge/account-linking), their
...
Neither switch affects [guild-to-guild bridging](/tribridge/guild-to-guild-bridging). Forwarded chat always uses real
names.
...
On the guild-chat side, a tester is recognised by their [account link](/tribridge/account-linking) — the bot has no
...
- **[officer channels](/tribridge/officer-chat)**, always. A channel that exists to record what officers said is the last
...
- **the real author is no longer visible on the message** — which is exactly why every disguised message is
  [audited](/tribridge/auditing).

Reposts within a channel are chained so a burst arrives in the order it was sent.

## Before you run one live

- Set an [audit channel](/tribridge/auditing). Without one, a disguised message has no record of who actually wrote it.
...
- [Auditing](/tribridge/auditing)
- [Admin panel](/tribridge/admin-panel)
