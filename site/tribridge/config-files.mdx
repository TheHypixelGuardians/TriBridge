---
title: "Config Files"
description: "Runtime config files the bot writes beside the repository."
---
...
|------------------------------|-----------------------------------------------------|--------------------------------------------------|
| `.env`                       | Token, bridge channel, log channel, first account   | Editing it — see [Configuration](/tribridge/configuration)  |
| `.minecraft-auth/`           | Cached Microsoft tokens, one entry per account      | The bot                                          |
| `guildsConfig.json`          | The Hypixel guild registry                          | [`/guilds`](/tribridge/hypixel-guilds)                      |
| `adminRolesConfig.json`      | Discord role ids that hold bot-admin                | [`/adminrole`](/tribridge/admin-roles)                      |
| `linkedAccountsConfig.json`  | Discord user → Minecraft name and UUID              | [`/link`, `/unlink`](/tribridge/account-linking)            |
| `linkRoleConfig.json`        | The role given to linked users                      | [`/linkrole`](/tribridge/link-role)                         |
| `auditChannelConfig.json`    | The default audit channel                           | [`/auditchannel`](/tribridge/auditing)                      |
| `globalProfileConfig.json`   | The running global profile change, and its scope    | [`/adminpanel`](/tribridge/global-profile-change)           |
| `featureRequestsConfig.json` | The request channel, the next id, and every request | [`/request`, `/requeststatus`](/tribridge/feature-requests) |
...
too, alongside `crossBridge`; see [Officer chat](/tribridge/officer-chat). `officerChannelId` is the one channel field
guilds may deliberately share — replies typed there are routed by [guild tag](/tribridge/guild-tags).
...
- [Hypixel guilds](/tribridge/hypixel-guilds)
