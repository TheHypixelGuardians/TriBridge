---
title: "Installation"
description: "Install TriBridge and connect your first Hypixel guild."
---
...
3. **Create the `.env` file** in the project root. Copy `.env.example` and fill it in:
   ...
   See [Configuration](/tribridge/configuration) for every variable and what it does; the minimum is:
...
4. **Invite the bot to your Discord server** with the permissions listed in [Permissions](/tribridge/permissions).
...
7. **Set the admin roles.**
   ...
   itself, and nothing else admin-gated will work. See [Admin roles](/tribridge/admin-roles).
...
See [Config files](/tribridge/config-files) for what each holds. They survive a `git pull`, which is what makes
[updating](/tribridge/updating) a two-command job.
...
The bot reconnects to Hypixel on its own ([Reconnection](/tribridge/reconnection)); it does not restart *itself* if the
