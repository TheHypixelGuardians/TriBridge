...
description: "Checklist for shipping a new slash command."
---
The checklist for a change that ships. [Architecture](/tribridge/architecture) explains why the pieces are shaped this
...
`handleCommands.js`. See [Admin roles](/tribridge/admin-roles).
...
1. **[CHANGELOG.md](https://github.com/TheHypixelGuardians/TriBridge/blob/master/CHANGELOG.md)** — an entry under
   `## Unreleased`, written for the people running the bot. Format in
   [Releasing](/tribridge/releasing).
2. **[DISCORD_CHANGELOG.md](https://github.com/TheHypixelGuardians/TriBridge/blob/master/DISCORD_CHANGELOG.md)** — one
   line, second person, only if a guild member would notice.
3. **[README.md](https://github.com/TheHypixelGuardians/TriBridge/blob/master/README.md)** — a row in the command table,
   and anything else it mentions that you changed.
4. **The public docs** —
   [docs/FEATURES.md](https://github.com/TheHypixelGuardians/TriBridge/blob/master/docs/FEATURES.md) is the canonical
   in-repo description; the Mintlify page restates it for its reader. A new command always touches
   [Commands](/tribridge/commands), which is meant to be exhaustive. New config file?
   [Config files](/tribridge/config-files). New permission? [Permissions](/tribridge/permissions).
...
- [Architecture](/tribridge/architecture)
