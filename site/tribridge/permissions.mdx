---
title: "Permissions"
description: "Discord permissions and intents required by TriBridge."
---
...
| **Add Reactions**   | The ❓ and 📡 [tag markers](/tribridge/guild-tags) are lost. Messages are still delivered     |
| **Manage Webhooks** | [Linked users](/tribridge/account-linking) relay the plain way, without their Minecraft face |
...
| [Audit channel](/tribridge/auditing)                                                 | View Channel, Send Messages, Embed Links                |
| [Request channel](/tribridge/feature-requests)                                       | View Channel, Send Messages, Embed Links                |
| [Officer channel](/tribridge/officer-chat)                                           | View Channel, Send Messages, Embed Links, Add Reactions |
| Every channel a [global profile change](/tribridge/global-profile-change) applies to | View Channel, Manage Webhooks, Manage Messages          |

An officer channel needs **Add Reactions** only for the routing markers — 📡 when a reply reached no guild,
❓ on an unrecognised [guild tag](/tribridge/guild-tags); without it the reply is still attempted. It deliberately needs
...
Officer channels are skipped by it whatever the permissions are — see [Officer chat](/tribridge/officer-chat).
...
|------------------|----------------------------------------------------------------------------------------|
| **Manage Roles** | Granting the [link role](/tribridge/link-role). The bot's own highest role must rank **above** it |
...
- **[Bot-admin roles](/tribridge/admin-roles)**, checked inside the command. Everything else admin-gated uses this.

And one guard that is neither: **every command is refused unless it came from the one Discord server the bot
serves.** See [Admin roles](/tribridge/admin-roles) for why that is not optional.
...
| `sky.shiiyu.moe` | [Networth](/tribridge/networth) figures, from SkyCrypt |
| `mc-heads.net`   | Player head images in embeds                |

## Next

- [Configuration](/tribridge/configuration)
- [Troubleshooting](/tribridge/troubleshooting)
