...
description: "Fix bridge, relay, and connection problems."
---
...
Messages** *and* **Embed Links**. See [Permissions](/tribridge/permissions).
...
Set `DISCORD_GUILD_ID` in the `.env` to the server's id and restart. See [Admin roles](/tribridge/admin-roles).

## Admin commands are refused for one person

They hold no [bot-admin role](/tribridge/admin-roles). `/adminrole add role:@Staff`, run by someone with the Discord
...
Move the bot's role above it and restart — the startup sync backfills everyone. See [Link role](/tribridge/link-role).
...
is never partially disguised. See [Global profile change](/tribridge/global-profile-change).
...
See [Networth](/tribridge/networth).
...
- **The [audit channel](/tribridge/auditing)** is a record for people, not an operational log.
