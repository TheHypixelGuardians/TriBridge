...
who else is an admin. One list serves both bots, so a role that counts as staff there counts as staff on the
bridge.

## What it gates on TriBridge

Everything under Management: `/invite`, `/kick`, `/promote`, `/demote`, `/send`, `/login`, `/guilds`,
[`/adminpanel`](Admin-Panel) and [`/auditchannel`](Auditing).

## It fails closed

A member with no matching role is not an admin — and neither is anybody at all when the shared database cannot
be reached, or when `DATABASE_URL` is not set. `/send` runs arbitrary commands as the Minecraft account and
`/adminpanel` can impersonate everybody in the server, so handing either out during a database outage would be
