...
- Node v22+. Dependencies: `discord.js`, `mineflayer`, `prismarine-auth`, `dotenv`, `pg`. `nodemon` is the
  only dev dependency, used by `npm run dev` and needed by nothing that runs the bot. `pg` talks to the
  community bot's database with plain SQL — there is deliberately no ORM and no schema on this side.
