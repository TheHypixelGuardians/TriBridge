...
const SCOPED_GUILD_OPTION = {
    ...GUILD_OPTION,
    description: 'Scope this to one Hypixel guild. Omit to change the default for all of them.',
};
...
async function handleShow(interaction) {
    ...
    const lines = [
        ...
        ...overrides.map((guild) => `> **${guild.name}** [${guild.tag}]: <#${guild.auditChannelId}>`),
    ];
...
async function handleClear(interaction, guildKey) {
    if (guildKey) {
        ...
        if (!guild.auditChannelId) {
            return interaction.reply(`⚠️ **${guild.name}** has no audit channel of its own.`);
        }
