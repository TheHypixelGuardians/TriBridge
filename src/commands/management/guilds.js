...
const FAILURE_MESSAGES = {
    ...
    'duplicate-account': 'Another guild already uses that Minecraft account. Two bots cannot share one account.',
...
function beginSignIn(guildKey, interaction) {
    ...
    createMcBot(guildKey, {onMsaCode: (info, guild) => requestSignIn(info, guild)})
...
module.exports = {
    ...
    options: [
        {
            name: 'list',
            description: 'List every registered Hypixel guild and its connection status.',
            type: ApplicationCommandOptionType.Subcommand,
        },
        {
            name: 'add',
            description: 'Register a Hypixel guild and sign its Minecraft account in.',
            type: ApplicationCommandOptionType.Subcommand,
            options: [
                {
                    name: 'key',
                    description: 'Short internal id, e.g. "sb". Lowercase, cannot be changed later.',
                ...
                {
                    name: 'tag',
                    description: 'Chat tag, 2-8 characters. Used as "!tag message" to target this guild.',
                    type: ApplicationCommandOptionType.String,
                    required: true,
                },
                {
                    name: 'account',
                    description: 'Microsoft account email for this guild\'s Minecraft bot.',
                    type: ApplicationCommandOptionType.String,
                    required: true,
                },
                {
...
async function handleList(interaction) {
    ...
    if (all.length === 0) {
        return interaction.reply({
            content: '⚠️ No Hypixel guilds are registered yet.\n' +
...
async function handleRemove(interaction) {
    ...
    if (!key) return interaction.reply({content: fail('unknown-key'), ephemeral: true});

    const guild = guilds.get(key);
    const isDefault = guilds.getDefault()?.key === key;

    if (isDefault && guilds.count() > 1 && !interaction.options.getBoolean('confirm')) {
        return interaction.reply({
            content: `⚠️ **${guild.name}** is the default guild.\n` +
    ...
    if (!result.ok) return interaction.reply({content: fail(result.reason), ephemeral: true});

    const next = guilds.getDefault();
    return interaction.reply({
        content: `✅ Removed **${guild.name}** and disconnected its bot.\n` +
            (next ? `> **${next.name}** is now the default guild.\n` : '> No guilds remain.\n') +
...
async function handleEdit(interaction) {
    ...
    for (const [optionName, field] of [['logchannel', 'logChannelId'], ['auditchannel', 'auditChannelId']]) {
...
async function handleDefault(interaction) {
    ...
    return interaction.reply({
        content: `✅ **${result.guild.name}** is now the default guild.\n` +
