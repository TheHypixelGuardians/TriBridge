...
module.exports = {
  ...
  options: [
    ...
    },
    {
      ...
      ...GUILD_OPTION,
      description:
        "Which Hypixel guild's bot to send from. Defaults to the default guild; never all.",
    },
  ],
  ...
  callback: async (client, interaction) => {
    ...
    const from = guilds.shouldShowTags()
      ? ` from **${target.guild.name}**`
      : "";

    if (combined) {
      const truncated =
        combined.length > 1900 ? combined.substring(0, 1900) + "…" : combined;
