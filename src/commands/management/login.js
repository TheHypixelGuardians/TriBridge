...
const {GUILD_OPTION, guildOptionAutocomplete} = require('../../utils/commandGuild');
...
module.exports = {
    ...
    options: [
        {
            ...GUILD_OPTION,
            description: 'Which Hypixel guild to connect. Defaults to every disconnected guild.',
        },
    ],
    ...
    callback: async (client, interaction) => {
        ...
        const results = await Promise.all(targets.map((guild) => connect(guild, interaction)));

        const lines = results.map(({guild, state, error}) => {
            if (state === 'connected') return `> ✅ **${guild.name}** — connected.`;
            if (state === 'busy') return `> 🔄 **${guild.name}** — a connection attempt is already in progress.`;
            if (state === 'signin') return `> 🔐 **${guild.name}** — needs a Microsoft sign-in; the code has been sent to you.`;
            if (state === 'failed') return `> ❌ **${guild.name}** — ${error}`;
            return `> 🔄 **${guild.name}** — still connecting; check \`/ping\` in a moment.`;
        });

        if (alreadyUp > 0) lines.push(`> ✅ ${alreadyUp} other guild(s) were already connected.`);
...
async function connect(guild, interaction) {
    ...
    const attempt = createMcBot(guild.key, {onMsaCode: (info, g) => requestSignIn(info, g)})
        .catch((error) => {
    ...
    if (record.lastError) return {guild, state: 'failed', error: record.lastError.message};
