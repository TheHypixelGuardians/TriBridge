...
async function handleProfile(interaction, action, invokerId) {
  ...
  if (action === "duration") {
    ...
    if (value === "custom")
      return showCustomDurationModal(interaction, invokerId);
  ...
  if (action === "startTest")
    return startEffect(interaction, "test", invokerId);
  if (action === "startLive")
    return startEffect(interaction, "live", invokerId);
...
async function handleScope(interaction, action, invokerId) {
  ...
  else if (action === "excludedChannels")
    setExcludedChannels(interaction.values);
...
module.exports = async (client, interaction) => {
  ...
  try {
    if (parsed.view === "main")
      return await handleMain(interaction, parsed.action, parsed.invokerId);
    if (parsed.view === "profile")
      return await handleProfile(interaction, parsed.action, parsed.invokerId);
    if (parsed.view === "scope")
      return await handleScope(interaction, parsed.action, parsed.invokerId);
  } catch (error) {
