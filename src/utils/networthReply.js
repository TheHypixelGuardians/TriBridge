...
const FAILURES = {
    ...
    'no-profiles': {
        ...
        embed: (d) => ({
            ...
            footer: 'TriBridge reads SkyCrypt\'s cache and cannot load a profile itself.',
        }),
    },
    'upstream-busy': {
    ...
    'upstream-error': {
        ...
        embed: (d) => ({
            ...
            lines: clip(d, 4) === '403'
                ? [`It returned HTTP ${clip(d, 4)}.`, 'It may be blocking automated requests right now.']
    ...
    'no-networth': {
        ...
        embed: (d) => ({
            emoji: '⚠️',
            title: 'No networth',
            lines: [`SkyCrypt has profiles for \`${clip(d, 40)}\` but no networth on any of them.`],
        }),
    },
    busy: {
...
function buildAnswerBody(result) {
    ...
    return `${player.name}: ${formatCoins(best.networth)} networth ` +
...
function buildChatAnswer(result) {
    ...
    return GC_PREFIX + (safe.length > CHAT_BUDGET ? safe.slice(0, CHAT_BUDGET) : safe);
}
...
function buildNetworthEmbed(result, context = {}) {
    ...
    if (best.noInventory) {
        embed.addFields({
            ...
            value: `> \`${player.name}\` has their inventory API disabled, so this is an ` +
    ...
    if (others) {
        embed.addFields({name: 'Other profiles', value: others.slice(0, MAX_FIELD_LENGTH)});
    }

    const footer = [attribution, 'cached up to 10 minutes'];
    if (context.asker) {
        const where = context.guild && guilds.shouldShowTags() ? ` in ${context.guild.name}` : '';
...
async function postToBridge(embed) {
    try {
        ...
        console.error('Failed to post a networth answer to the bridge channel:', error);
    }
}
