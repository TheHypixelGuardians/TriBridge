...
function loadConfig() {
    ...
    cachedConfig.guilds = cachedConfig.guilds.filter((g) => g && typeof g === 'object');
...
function getEnabled() {
    return loadConfig().guilds.filter((g) => g.enabled !== false).map(copy);
}
...
function getByTag(tag) {
    ...
    return copy(loadConfig().guilds.find((g) => String(g.tag).toLowerCase() === wanted) ?? null);
}
...
function add(input) {
    ...
    const config = loadConfig();
    if (config.guilds.some((g) => g.key === key)) return {ok: false, reason: 'duplicate-key'};
    if (config.guilds.some((g) => String(g.tag).toLowerCase() === tag.toLowerCase())) {
...
function update(key, patch) {
    const config = loadConfig();
    const guild = config.guilds.find((g) => g.key === String(key ?? '').toLowerCase());
    ...
    if (Object.keys(changes).length === 0) return {ok: false, reason: 'no-changes'};
...
function remove(key) {
    ...
    if (config.defaultKey === removed.key) {
        config.defaultKey = config.guilds.find((g) => g.enabled !== false)?.key
            ?? config.guilds[0]?.key
            ?? null;
    }
