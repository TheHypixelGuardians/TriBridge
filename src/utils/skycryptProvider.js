...
function classify(message, status) {
    if (/no skyblock profiles/i.test(message)) return {ok: false, code: 'no-profiles'};
    if (/rate limit|too many/i.test(message)) return {ok: false, code: 'upstream-busy'};
    if (status === 429 || status === 503) return {ok: false, code: 'upstream-busy'};
...
async function fetchNetworth(player) {
    ...
    let response;
    try {
        response = await fetch(SKYCRYPT_PROFILE_URL + encodeURIComponent(username), {
        ...
        return {ok: false, code: 'network', detail: error?.message ?? String(error)};
    }
