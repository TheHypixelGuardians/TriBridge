...
function retireBot(record) {
    ...
    for (const eventName of ['spawn', 'end', 'kicked', 'message', 'login', 'error']) {
...
async function createMcBot(guildKey, options = {}) {
    ...
    if (guild.enabled === false) throw new Error(`Hypixel guild "${guildKey}" is disabled.`);
    ...
    let bot;
    try {
        const flow = new Authflow(guild.account, PROFILES_FOLDER, AUTH_OPTIONS, codeCallback);
    ...
    bot.on('error', (error) => {
        record.lastError = error;
        console.error(`[${guildKey}] Minecraft bot error:`, error?.message ?? error);
    });
    ...
    bot.on('spawn', () => {
        ...
        if (wasDown) {
            void logForGuild(guildKey, `✅ Connected to Hypixel as \`${bot.username}\`.`);
        }
    });
    ...
    bot.on('kicked', (reason) => {
        ...
        void logForGuild(guildKey, '⚠️ Kicked from Hypixel. Check the console for the reason.');
    });
    ...
    const watchdog = setTimeout(() => {
        if (record.bot !== bot || record.connected || record.awaitingDeviceCode) return;
