...
function buildProfileEmbed(draft) {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "Duration",
        value: describeDuration(draft.duration),
        inline: true,
      },
      {
...
function buildProfileRows(invokerId, draft) {
  ...
  const toggleRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setLabel(
        `Discord → Minecraft: ${state.disguiseToMinecraft ? "on" : "off"}`,
      )
      .setEmoji(state.disguiseToMinecraft ? "🎭" : "🚫")
      .setStyle(
        state.disguiseToMinecraft ? ButtonStyle.Success : ButtonStyle.Secondary,
      ),
    new ButtonBuilder()
      .setCustomId(customId("profile", "toggleDiscord", invokerId))
      .setLabel(
        `Minecraft → Discord: ${state.disguiseToDiscord ? "on" : "off"}`,
      )
      .setEmoji(state.disguiseToDiscord ? "🎭" : "🚫")
      .setStyle(
        state.disguiseToDiscord ? ButtonStyle.Success : ButtonStyle.Secondary,
      ),
  );
...
function buildScopeRows(invokerId) {
  ...
  if (state.testChannelIds.length > 0) {
    testChannelSelect.setDefaultChannels(
      state.testChannelIds.slice(0, MAX_DEFAULTS),
    );
  }
  if (state.excludedChannelIds.length > 0) {
    excludedSelect.setDefaultChannels(
      state.excludedChannelIds.slice(0, MAX_DEFAULTS),
    );
  }
...
function profileView(invokerId) {
  ...
  return {
    embeds: [buildProfileEmbed(draft)],
    components: buildProfileRows(invokerId, draft),
  };
}
